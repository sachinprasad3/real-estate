    ...
    <table id="example1" class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $post->price }}</td>
          <td> <a href="{{ route('seller_post.edit',$post->id) }} "><i class="fas fa-edit"></i></a> </td>
          <td>
            <form id="delete-form-{{ $post->id }}" action="{{ route('seller_post.destroy',$post->id) }}" method="post" style="display:none">
              {{csrf_field()}}
              {{method_field('DELETE')}}
